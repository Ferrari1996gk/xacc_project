...
int main(int argc, const char* argv[]) {
    ...
    //CPU calculation
    std::cout << "CPU monte carlo path number: " << 1024*4 << std::endl;
	struct timeval cpu_st_time, cpu_end_time;
	gettimeofday(&cpu_st_time, 0);
	double cpu_value;
	cpu_value = monte_carlo_put_price(1024*4, underlying, strike, riskFreeRate, volatility, timeLength);
	std::cout << "cpu result = " << cpu_value << std::endl;
	gettimeofday(&cpu_end_time, 0);
	exec_time = tvdiff(&cpu_st_time, &cpu_end_time);
	time_elapsed = double(exec_time) / 1000;
	std::cout << "CPU execution time(ms): " << time_elapsed << " ms\n";
    //
